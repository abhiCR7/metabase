(ns metabase.driver.rest.query-processor
  (:require [cheshire.core :as json]
            [clojure.core.match :refer [match]]
            [clojure.math.numeric-tower :as math]
            [clojure.string :as s]
            [clojure.tools.logging :as log]
            [metabase.driver.rest.js :as js]
...
(extend-protocol IRValue
  ...
  DateTimeValue         (->rvalue [{{unit :unit} :field, value :value}] ((partial u/format-date "yyyy-MMM-dd HH:mm:ss") (u/date-trunc unit value (get-timezone-id))))
...
(defprotocol ^:private IBaseType
  (^:private get-base-type [this]
    "Is this `Field`/`DateTimeField` a `:dimension` or `:metric`?"))

(extend-protocol IBaseType
  Field         (get-base-type [{:keys [base-type]}]
                  base-type)

  DateTimeField (get-base-type [{:keys [base-type]}]
                  base-type))
...
(defn- parse-filter-subclause:filter [{:keys [filter-type field value] :as filter}]
  (log/warn (u/format-color 'red (str "PARSING FILTER " filter-type " " field " " value) ))
  (let [field (when field (->lvalue field))
        value (when value (->rvalue value))
        v     (case filter-type
                :between     {"range" {field {"from" (->rvalue (:min-val filter))
                                            "to" (->rvalue (:max-val filter))}}}
                :contains    {"terms" {field (vec value)}}
    ...
    (log/warn (u/format-color 'red (str "values post FILTER " v) ))
    v
  )
  )
...
(defn- parse-filter-clause:filter [{:keys [compound-type subclauses subclause], :as clause}]
  {:pre [clause]}
  (log/warn (u/format-color 'red (str "PARSING FILTER " compound-type subclauses subclause) ))
  (case compound-type
    :and {:and (filterv identity (map parse-filter-clause:filter subclauses))}
    :or  {:or (filterv identity (map parse-filter-clause:filter subclauses))}
...
(defn- handle-filter [_ {filter-clause :filter} druid-query]
  (when filter-clause
    (let [filter    (parse-filter-clause:filter    filter-clause)
          out {"query" {"constant_score" {"filter" { "bool" {"must" filter}}}}}
          ]

      (log/warn (u/format-color 'red "WARNING: Final set of filters: %s" out))
      out)))


(defn- handle-breakout [_ {breakout-fields :breakout} druid-query]
  (let [v  (for [field breakout-fields]
                            (cond
                               (isa? (get-base-type field) :type/Text)    {:clause {"terms" {"field" (->lvalue field )}}  :name (->lvalue field)}
                               (instance? DateTimeField field)   {:clause {"date_histogram" {"field" (->lvalue field ) "interval" "day"} } :name (->lvalue field)}
                               (isa? (get-base-type field) :type/Integer) {:clause {"terms" {"field" (->lvalue field )}}  :name (->lvalue field)}
                               (isa? (get-base-type field) :type/Boolean)    {:clause {"terms" {"field" (->lvalue field )}}  :name (->lvalue field)}))
        out (reduce (fn [agg,expr] {"aggs" { (:name expr) (merge (:clause expr) agg) }} ) {} v) ]
    (log/warn (u/format-color 'red "WARNING: Intermediate set of breakouts: %s" v))

    (log/warn (u/format-color 'red "WARNING: Final set of breakouts: %s" out))
     (merge {"size" 0} out)  ))
...
;  {:pre [(map? query)]}
;  (log/warn (u/format-color 'red (str "WARNING: PRINTING QUERY " query) ))
...
(defn- build-druid-query
  ...
  [query]
  (let [query-type (druid-query-type query)]
  (loop [es-query {"size":10000}, [f & more] [
                                  handle-filter
                                  handle-breakout]]
    (let [out      (f query-type query es-query)
          es-query (if
                     (nil? out)        es-query (merge es-query out))]
      (if-not (seq more)
        ["placeholder" es-query]
        (recur es-query more))))))
...
(defn post-process-native
  ...
  {:pre [queryType]}
  (post-process (keyword "metabase.driver.rest.query-processor" (name queryType))
                results))
...
(defn post-process-elastic [_  {:keys [columns rows]}]
...
(defn loghelper [data] (log/warn (u/format-color 'red (str "output returned by elastic   " data) ) ) data)
